...
// tmux command - use env var or fall back to PATH lookup
 ...
 */
function createSession(name, directory) {
  ...
  const baseDir = process.env.CLAUDEPOD_PROJECTS_DIR || path.join(os.homedir(), 'Projects');
  ...
  execSync(`${TMUX} new-session -d -s ${escapeSessionName(sessionName)} -c "${workingDir}" 'claude'`, {
    encoding: 'utf8',
    timeout: 10000
  });
