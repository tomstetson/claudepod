...
function createSession(name) {
  ...
  // Create detached session running claude in projects directory
  const projectsDir = process.env.CLAUDEPOD_PROJECTS_DIR || '/Users/tomstetson/Projects';
  execSync(`tmux new-session -d -s ${escapeSessionName(sessionName)} -c "${projectsDir}" 'claude'`, {
    encoding: 'utf8',
    timeout: 10000
  });
