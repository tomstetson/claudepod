...
/**
 * Set notification preference for a session
 * @param {string} sessionName
 * @param {boolean} enabled
 */
function setNotifications(sessionName, enabled) {
  const meta = loadMeta();
  if (!meta[sessionName]) {
    meta[sessionName] = {};
  }
  meta[sessionName].notifications = enabled;
  meta[sessionName].updatedAt = new Date().toISOString();
  saveMeta(meta);
}

/**
 * Get notification preference for a session (default: true)
 */
function getNotifications(sessionName) {
  const meta = loadMeta();
  const pref = meta[sessionName]?.notifications;
